...
The query parameters are identical to the [Current weather endpoint](../reference/current-weather). You can query by `q` (city name) or `lat` / `lon` (coordinates).
...
The response structure is slightly different from the current weather endpoint. Instead of a single object, you get a `list` array containing 40 objects. The example below shows only the first item; the real response contains all 40.
